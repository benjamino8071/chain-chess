...
public class LevBlackSystem : LevDependency
{
    ...
    public override void GameStart(LevCreator levCreator)
    {
        ...
        List<Transform> blackSps = levCreator.GetObjectsByName(AllTagNames.BlackSp);
        foreach (Transform blackSp in blackSps)
        {
            PieceType pieceType = blackSp.GetComponent<PieceType>();
            
            LevPieceController levPlayerController = Creator.blackControlledBy == ControlledBy.Player 
                ? new LevPlayerController() : new LevAIController();
            levPlayerController.GameStart(levCreator);
            levPlayerController.Init(blackSp, pieceType.piece, LevPieceController.PieceColour.Black);
    ...
    public override void GameUpdate(float dt)
    {
        ...
        if (Creator.inputSo._leftMouseButton.action.WasPerformedThisFrame() && Creator.blackControlledBy == ControlledBy.Player)
        {
            /*
    ...
    public void PieceCaptured(LevPieceController pieceController)
    {
        ...
        {
            _whiteSystem.SetStateForAllPieces(LevPieceController.States.EndGame);
            SetStateForAllPieces(LevPieceController.States.EndGame);
            _endGameSystem.SetEndGame(LevPieceController.PieceColour.White);
        }
    }
